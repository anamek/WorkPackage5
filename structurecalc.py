...
def get_prop_vol(dry_mass, Delta_V, vol_margin):
    ...
    V_ox = m_ox / rho_ox
    return V_fuel * vol_margin, V_ox * vol_margin, m_ox, m_fuel
...
def sigma_tot(R, L, t, gy, gx, M):
    ...
    return (sigma_bend + sigma_norm)
    
def t_for_buckling(R, L, E, M, SF):
    ...
    for i in range(10):
        ...
        diffs = np.abs(sigma_tot(R, L, test_ts, 3 * 9.81, 4.1 * 9.81, M)*SF - crit_buckling_strength(R, L, test_ts, E))
...
def get_dims(m_tot_dry, mat, SF):
    ...
    *prop_vols, m_ox, m_fuel = get_prop_vol(m_tot_dry, Delta_V_req,tank_vol_margin)
    ...
    R_struct = math.ceil((R_tank_fuel + 0.01)*100)/100
    ...
    t_struct = t_for_buckling(R_struct, L_struct , mat["E"], m_tot_dry + m_prop, SF)
    m_struct = 2 * math.pi * R_struct * t_struct * L_struct * mat["rho"]
    return(m_struct, m_ox, m_fuel, R_struct, t_struct, R_tank_fuel, L_tank_ox, L_struct) 
